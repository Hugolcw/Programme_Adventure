...
**阶乘**（`computer_guide/function_exercises/factorial.cpp`）：
- 递归终止条件：n == 0 或 n == 1
- 递归关系：fact(n) = n * fact(n-1)
- 递归调用：调用自身

**汉诺塔**（`computer_guide/function_exercises/hanoi_tower.cpp`）：
- 递归分解：将n个盘子的问题分解为两个n-1的问题
- 递归关系：hanoi(n) = hanoi(n-1) + 移动 + hanoi(n-1)
- 参数传递：from, to, by三个参数

**猴子吃桃（递归）**（`computer_guide/function_exercises/recursive_monkey_peach.cpp`）：
- 递归终止：n == 1
...
**海绵城市（BFS）**（`computer_guide/function_exercises/two_dimensional_array_sponge_city.cpp`）：
- 使用数组模拟队列：`int q[N + 5]`
...
**来源**：将`computer_guide`中的迭代实现改为递归
...
8. **参考题目**：猴子吃桃（递归） - 递推递归
   - 位置：`computer_guide/function_exercises/recursive_monkey_peach.cpp`
   - 说明：作为递推递归的典型参考

#### 其他参考题目
- **阶乘**：`computer_guide/function_exercises/factorial.cpp` - 基础递归参考
- **汉诺塔**：`computer_guide/function_exercises/hanoi_tower.cpp` - 递归分解参考
...
#### 参考题目
- **图的连通性判断（海绵城市）**：`computer_guide/function_exercises/two_dimensional_array_sponge_city.cpp` - BFS标准实现参考
